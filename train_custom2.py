...
import sys
import shutil
from datetime import datetime  # Added for descriptive folder naming
...
def save_to_drive(src_path, drive_root_folder):
    ...
    try:
        ...
        # The destination is now the root folder itself
        dst = os.path.join(drive_base, drive_root_folder)
        os.makedirs(dst, exist_ok=True) # Ensure the root folder exists

        if src.is_dir():
            # Copy directory into the root folder
            ...
            # Copy file into the root folder
...
def main(opt):
    ...
    # --- FINAL CHANGE: CONSTRUCT DESCRIPTIVE FOLDER NAME FOR GOOGLE DRIVE ---
    ...
    if opt.save_to_drive:
        ...
        prune_percent_str = f'{prune_percent:g}' # Format to remove trailing .0

        descriptive_name = (
            f"{date_str}_{model_name}_pruned{prune_percent_str}pct_"
            f"retrain{opt.pruning_epoch}e_final{opt.total_epochs}e"
        )
        # The final path is the user-provided base path + our descriptive folder name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/custom_train', help='local save directory for results')
